/* Search funtion for web app. Class MIU. Term 1306.*/
window.addEventListener("DOMContentLoaded", function () {
        ...
        function tn(x) {
            var elementTn = getElementByTagName(x);
            return elementTn;
        }
        ...
        var getSearch = function () {
           ...
           /* if (choreType != "--Choose A Location--" && term === "") {
            ...
            // Search by term
            if (myTypeArray.value === "--Choose A Location--" && term != "") {
                var makeList = document.createElement("ul");
                document.getElementById("results").appendChild(makeList);
                for (var i = 0, len = localStorage.length; i < len; i++) {
                    var key = localStorage.key(i);
        ...
        searchButton1.addEventListener("click", buttonSearch);
